...
namespace WarehouseManagement.Model
{
    ...
    { 
        ...
        public bool Sua_HangHoa(string New_MAHANG, string TENHANG, string DVT, int SOLUONG, float DONGIA, string NGNHAP, string NGSANXUAT, string HSD, string MAHANG)
        {
            string sql_update = "UPDATE HANGHOA SET MAHANG = '" + New_MAHANG + "', TENHANG ='" 
                + TENHANG + "', DVT = '" + DVT + "',SOLUONG = " + SOLUONG + ", DONGIA = " 
                + DONGIA + ", NGNHAP = '" + NGNHAP + "', NGSANXUAT = '" + NGSANXUAT + "' , HSD = '" 
                + HSD + "' WHERE MAHANG = '" + MAHANG + "'";
            bool re = false;
        ...
        {
            DataTable re;
            string sql_select = "Select MAHANG,TENHANG,DVT,SOLUONG,DONGIA,NGNHAP,NGSANXUAT,HSD from HANGHOA where MAHANG = '" + MAHANG+ "' AND UKEY ="+ UKEY;
            re = Process.createTable(sql_select);
            return re;  //Trả về 1 DataTable
        }
        ...
        {
            DataTable re;
            string sql_select = "Select MAHANG,TENHANG,DVT,SOLUONG,DONGIA,NGNHAP,NGSANXUAT,HSD from HANGHOA where TENHANG = '" + TENHANG + "' AND UKEY =" + UKEY;
            re = Process.createTable(sql_select);
            return re;  //Trả về 1 DataTable
        }
        ...
        {
            DataTable re;
            string sql_select = "Select MAHANG,TENHANG,DVT,SOLUONG,DONGIA,NGNHAP,NGSANXUAT,HSD from HANGHOA where cast (NGNHAP as date) = '" + NGNHAP + "' AND UKEY =" + UKEY;
            re = Process.createTable(sql_select);
            return re;  //Trả về 1 DataTable
        }
    }

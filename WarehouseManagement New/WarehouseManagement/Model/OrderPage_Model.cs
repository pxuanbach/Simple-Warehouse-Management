...
namespace WarehouseManagement.Model
{
    class OrderPage_Model
    {
        ...
        {
            string sql_select = "Select MAHANG, SOLUONG From CTHD Where MAHOADON = " + MAHD + " and UKEY = " + UKEY;

            return Process.GetCTHDs(sql_select);
        }

        public bool Add_CTHD(int MAHD, string MAHANG, int SOLUONG, int DONGIA, int UKEY)
        {
            string sql_insert = "insert CTHD values (" + MAHD + ", '" + MAHANG + "', " + SOLUONG + ", " + DONGIA + ", " + UKEY + ")";
            bool re = false;
        ...
        public DataTable Load_TENHANG(string TENHANG, int UKEY)   //Đầu vào cần 1 mã Ukey và tên hàng hóa
        {
            DataTable re;
            string sql_select = "Select MAHANG,TENHANG,SOLUONG,DONGIA,NGNHAP from HANGHOA " +
                "where CHARINDEX(N'" + TENHANG + "', TENHANG) != 0 AND UKEY =" + UKEY;
            re = Process.createTable(sql_select);
            return re;  //Trả về 1 DataTable
        }

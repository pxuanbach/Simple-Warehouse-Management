    ...
    public partial class FHistory : Form
    {
        ...
        void cusTomDGV(int Type)
        {
            ...
            if (Type == 1)
            {
                ...
                dGVHistory.Columns.Add("LIENHE", "Liên Hệ");
                dGVHistory.Columns["LIENHE"].DataPropertyName = "LIENHE";
        ...
        void PosDefine_dGV()
        {
            if (dGVHistory.SelectedRows.Count > 0)
            {
                ...
                {
                    int currentRow = dGVHistory.SelectedRows[0].Index;
                    hoaDon.MaHD = Convert.ToInt32(dGVHistory.Rows[currentRow].Cells["MAHD"].Value);
                    hoaDon.TenDoiTac = dGVHistory.Rows[currentRow].Cells["TENDT"].Value.ToString();
                    hoaDon.LienHe = dGVHistory.Rows[currentRow].Cells["LIENHE"].Value.ToString();
                    hoaDon.ThoiGian = DateTime.ParseExact(dGVHistory.Rows[currentRow].Cells["NGAYNHAP"].Value.ToString(), "dd/MM/yyyy", System.Globalization.CultureInfo.InvariantCulture);
                    hoaDon.GiaTri = dGVHistory.Rows[currentRow].Cells["GIA"].Value.ToString();
                    hoaDon.MaNguoiThem = Convert.ToInt32(dGVHistory.Rows[currentRow].Cells["KEY"].Value);
                    hoaDon.TenNguoiThem = dGVHistory.Rows[currentRow].Cells["TEN"].Value.ToString();

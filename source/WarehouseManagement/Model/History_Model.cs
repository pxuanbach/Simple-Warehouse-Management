...
namespace WarehouseManagement.Model
{
    class History_Model
    {
        public DataTable Load_On_QuanLy(string dd, string MM, string yyyy, int Type)   //Đầu vào cần 1 mã Ukey 
        {
            ...
            {
                sql_select = "Select HOADON.MAHOADON, TENDOITAC, LIENHE, CONVERT(varchar,HOADON.THOIGIAN, 103) as THOIGIAN, TRIGIA, USERACC.UKEY, HOTEN " +
                            "From HOADON inner join USERACC on HOADON.UKEY = USERACC.UKEY ";
                sql_where = "Where DAY(HOADON.THOIGIAN) = " + dd +
                            " and Year(HOADON.THOIGIAN) = " + yyyy + " and Month(HOADON.THOIGIAN) = " + MM + " and HOADON.TRIGIA > 0";
            }    
